...
def conloss (anchor_features, contrast_feature, anchor_labels, contrast_labels, P=None,temperature=0.7,loss_weight=0.05):
        """
        Args:
...
def backbone_loss(features, prev_features):
    ...
    loss = feature_distillation(features['p4'], prev_features['p4'])
    loss += feature_distillation(features['p5'], prev_features['p5'])
...
def feature_distillation(features, prev_features):
    ...
    improved adaptative
    """
    # index=torch.where(param<0.5)
    # prev_features[index]=features[index]
