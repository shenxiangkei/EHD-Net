...
from ..distillation_loss import roi_head_loss,conloss
...
class ROIHeads(torch.nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        # self.enable_thresold_autolabelling = cfg.MODEL.enable_thresold_autolabelling
        # fmt: on
    ...
    def _sample_proposals(self, matched_idxs, matched_labels, gt_classes,objectness_logits):
        """
        ...
        #     *heapq.nlargest(1, enumerate(pred_objectness_score_ss), key=operator.itemgetter(1))))[0]
...
class Res5ROIHeads(ROIHeads):
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        if self.base_model is not None and self.enable_roi_distillation:
            ...
            prev_pred_class_logits=None
            prev_pred_proposal_deltas=None
        outputs = FastRCNNOutputs(
            self.box2box_transform,
            pred_class_logits,
            pred_proposal_deltas,
...
#
    ...
    def get_predictions_from_boxes(self, boxes):
        old_features=boxes.clone()
        feature_pooled = self.box_head(boxes)
        pred_class_logits, pred_proposal_deltas = self.box_predictor(feature_pooled)
        return pred_class_logits, pred_proposal_deltas,old_features
    ...
    def _forward_box(self, features, proposals):
        ...
        proposal_boxes = [x.proposal_boxes for x in proposals]
        boxes = self._shared_roi_transform(features, proposal_boxes)
        box_features = self.box_pooler(features, [x.proposal_boxes for x in proposals])
        new_features = box_features.clone()
        box_features = self.box_head(box_features)
        pred_class_logits, pred_proposal_deltas = self.box_predictor(box_features)

        if self.base_model is not None and self.enable_roi_distillation:
            prev_pred_class_logits, prev_pred_proposal_deltas,old_features = self.base_model.roi_heads. \
                get_predictions_from_boxes(boxes)
        else:
            prev_pred_class_logits=None
            prev_pred_proposal_deltas=None
        ...
        if self.training:
            losses= outputs.losses()
            if self.base_model is not None and self.enable_roi_distillation:
                label_n = cat([x.gt_classes for x in proposals], dim=0)
                B,N,h,w=new_features.shape
                new_features=new_features.permute(0,2,3,1)
                new_features=new_features.reshape(B,h*w,N)
                num = prev_pred_class_logits.shape[0]
                mask_new_classes = label_n.view(num) < self.num_classes
                min_new_classes = label_n.view(num)[mask_new_classes].min()
                label_mix = label_n.clone()
                _, label_po = prev_pred_class_logits.max(dim=1)
                label_mix[label_mix.clone() == self.num_classes] = label_po[label_mix.clone() == self.num_classes]
                label_ = label_mix
                Lable_anchor = label_[label_ < self.num_classes].clone()
                Lable_contrast = torch.cat((Lable_anchor, label_[(label_ < self.num_classes) & ~mask_new_classes]))
                old_features=old_features.detach().permute(0,2,3,1)
                old_features=old_features.reshape(B,h*w,N)
                Output_anchor = F.normalize(new_features.reshape(B,h*w*N)[label_ < self.num_classes], dim=1)
                Output_contrast = torch.cat(
                    (Output_anchor, F.normalize(old_features.reshape(B,h*w*N)[(label_ < self.num_classes) & ~mask_new_classes], dim=1)),
                    dim=0)
                ## make joint probality mask
                l_po_ = torch.softmax(prev_pred_class_logits, dim=-1)
                l_po_anchor = l_po_[label_ < self.num_classes]
                l_po_contrast = torch.cat((
                    l_po_[label_ < self.num_classes],
                    l_po_[(label_ < self.num_classes) & ~mask_new_classes]))
                ...
                mask_anchor_jp = mask_anchor_jp.unsqueeze(dim=1).float()
                mask_contrast_jp = mask_contrast_jp.unsqueeze(dim=1).float()
                M_gt = torch.mm(mask_anchor_jp, mask_contrast_jp.T)
                JM_p[M_gt == 1] = 1
                contrast_loss=conloss(Output_anchor,Output_contrast,Lable_anchor,Lable_contrast,JM_p.detach())
            ...
            if self.base_model is not None and self.enable_roi_distillation:
                if self.distill_only_fg_roi:
                    ...
                    prev_pred_class_logits, prev_pred_proposal_deltas,_ = self.base_model.roi_heads. \
                        get_predictions_from_boxes(boxes)
                roi_dist_loss = roi_head_loss(pred_class_logits[:, 0:self.num_base_class], pred_proposal_deltas,
                                              prev_pred_class_logits[:, 0:self.num_base_class],
                                              prev_pred_proposal_deltas, self.dist_loss_weight)
